<template>
  ...
  <div class="slider">
    <vue-slider v-model="sliderValues" :width="300" :height="8" 
      :minRange="1" :min="2000" :max='2021' :enable-cross="false" :lazy="true"  :marks="value => value % 5 === 0 ? {labelStyle: { color: '#ccc' }} : false" 
      :process-style="{ backgroundColor: '#003366' }" :tooltip-style="{ backgroundColor: '#003366', borderColor: '#9fbbd1' }"
  ...
  <div class="playerDashboard"> 
      <div class="chart left">
        <BarChart :chartData="leftChartData" :options="leftChartOptions" :styles="myStyles" />
      </div>
      <div class="chart right">
        <LineChart :chartData="rightChartData" :options="rightChartOptions" :styles="myStyles" />
      </div>
  </div>
...
import { BarChart, LineChart, ExtractComponentData} from 'vue-chart-3';
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      blocks: [] as number[],
      seasons: [] as number[],
      leftChartData: {labels : [] as any[],
        datasets:[] as any[],
        },
      rightChartData: {labels : [] as any[],
        datasets:[] as any[],
        },
      leftChartOptions: {
          animation: false,
  ...
  methods: {
    async getPlayerStats(seasonStart: number, seasonEnd: number) {
        ...
        for( i = seasonStart; i <= seasonEnd; i++){
          await axios
            .get("https://www.balldontlie.io/api/v1/season_averages?season=" + i + "&player_ids[]=" +this.$route.params.id)
